...
inline void h_numeric_full_occu(const CSR &A, const CSR &B, CSR &C, Meta &meta)
{
    ...
    if (meta.bin_size[6])
    {
        CHECK_ERROR(cudaFuncSetAttribute(k_numeric_max_shared_hash_tb_half_occu,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize, 98304));
        k_numeric_max_shared_hash_tb_half_occu<<<meta.bin_size[6], 1024, 98304, meta.stream[6]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
                                                                                                  meta.d_bins + meta.bin_offset[6],
    ...
    { // global bin
        ...
        k_numeric_global_hash_tb_full_occu<<<meta.bin_size[7], 1024, 0, meta.stream[7]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
    ...
    if (meta.bin_size[5])
    {
        k_numeric_shared_hash_tb_full_occu<4096, 1024>
            <<<meta.bin_size[5], 1024, 0, meta.stream[5]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
                                                            meta.d_bins + meta.bin_offset[5],
                                                            C.d_rpt, C.d_col, C.d_val);
    }
    if (meta.bin_size[0])
    ...
    if (meta.bin_size[4])
    {
        k_numeric_shared_hash_tb_full_occu<2048, 512>
            <<<meta.bin_size[4], 512, 0, meta.stream[4]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
                                                           meta.d_bins + meta.bin_offset[4],
                                                           C.d_rpt, C.d_col, C.d_val);
    }
    if (meta.bin_size[3])
    {
        k_numeric_shared_hash_tb_full_occu<1024, 256>
            <<<meta.bin_size[3], 256, 0, meta.stream[3]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
                                                           meta.d_bins + meta.bin_offset[3],
                                                           C.d_rpt, C.d_col, C.d_val);
    }
    ...
    if (meta.bin_size[2])
    {
        k_numeric_shared_hash_tb_full_occu<512, 128>
            <<<meta.bin_size[2], 128, 0, meta.stream[2]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
                                                           meta.d_bins + meta.bin_offset[2],
                                                           C.d_rpt, C.d_col, C.d_val);
    }
    if (meta.bin_size[1])
    {
        k_numeric_shared_hash_tb_full_occu<256, 64>
            <<<meta.bin_size[1], 64, 0, meta.stream[1]>>>(A.d_rpt, A.d_col, A.d_val, B.d_rpt, B.d_col, B.d_val,
                                                          meta.d_bins + meta.bin_offset[1],
                                                          C.d_rpt, C.d_col, C.d_val);
    }
