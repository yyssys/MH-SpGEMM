    ...
    for (i = BIN_NUM - 1; i >= 0; i--)
    {
        if (bin->bin_size[i] > 0)
        {
            ...
            case 0:
                ...
                set_row_nz_bin_pwarp<<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol,
                                                                    d_brpt, d_bcol,
                                                                    bin->d_row_perm,
                                                                    bin->d_row_nz,
                                                                    bin->bin_offset[i],
            ...
            case 1:
                BS = 64;
                GS = bin->bin_size[i];
                set_row_nz_bin_each_tb<512><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
            ...
            case 2:
                BS = 128;
                GS = bin->bin_size[i];
                set_row_nz_bin_each_tb<1024><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
                                                                            bin->d_row_perm, bin->d_row_nz,
                                                                            bin->bin_offset[i], bin->bin_size[i]);
                break;
            case 3:
                BS = 256;
                GS = bin->bin_size[i];
                set_row_nz_bin_each_tb<2048><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
                                                                            bin->d_row_perm, bin->d_row_nz,
                                                                            bin->bin_offset[i], bin->bin_size[i]);
                break;
            case 4:
                BS = 512;
                GS = bin->bin_size[i];
                set_row_nz_bin_each_tb<4096><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
                                                                            bin->d_row_perm, bin->d_row_nz,
                                                                            bin->bin_offset[i], bin->bin_size[i]);
                break;
            case 5:
                BS = 1024;
                GS = bin->bin_size[i];
                set_row_nz_bin_each_tb<8192><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
                                                                            bin->d_row_perm, bin->d_row_nz,
                                                                            bin->bin_offset[i], bin->bin_size[i]);
                break;
            case 6:
            {
                ...
                BS = 1024;
                GS = bin->bin_size[i];
                set_row_nz_bin_each_tb_large<8192><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
                                                                                  bin->d_row_perm, bin->d_row_nz,
                ...
                if (fail_count > 0)
                {
                    ...
                    GS = bin->bin_size[i];
                    set_row_nz_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_brpt, d_bcol,
    ...
    for (i = BIN_NUM - 1; i >= 0; i--)
    {
        if (bin->bin_size[i] > 0)
        {
            ...
            case 0:
                BS = 256;
                GS = div_round_up(bin->bin_size[i] * PWARP, BS);
                calculate_value_col_bin_pwarp<<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
            ...
            case 1:
                BS = 64;
                GS = bin->bin_size[i];
                calculate_value_col_bin_each_tb<256><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
                                                                                    d_brpt, d_bcol, d_bval,
                                                                                    d_crpt, d_ccol, d_cval,
                                                                                    bin->d_row_perm, bin->d_row_nz,
                                                                                    bin->bin_offset[i], bin->bin_size[i]);
            ...
            case 2:
                BS = 128;
                GS = bin->bin_size[i];
                calculate_value_col_bin_each_tb<512><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
                                                                                    d_brpt, d_bcol, d_bval,
                                                                                    d_crpt, d_ccol, d_cval,
                                                                                    bin->d_row_perm, bin->d_row_nz,
                                                                                    bin->bin_offset[i], bin->bin_size[i]);
            ...
            case 3:
                BS = 256;
                GS = bin->bin_size[i];
                calculate_value_col_bin_each_tb<1024><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
                                                                                     d_brpt, d_bcol, d_bval,
                                                                                     d_crpt, d_ccol, d_cval,
                                                                                     bin->d_row_perm, bin->d_row_nz,
                                                                                     bin->bin_offset[i], bin->bin_size[i]);
            ...
            case 4:
                BS = 512;
                GS = bin->bin_size[i];
                calculate_value_col_bin_each_tb<2048><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
                                                                                     d_brpt, d_bcol, d_bval,
                                                                                     d_crpt, d_ccol, d_cval,
                                                                                     bin->d_row_perm, bin->d_row_nz,
                                                                                     bin->bin_offset[i], bin->bin_size[i]);
            ...
            case 5:
                BS = 1024;
                GS = bin->bin_size[i];
                calculate_value_col_bin_each_tb<4096><<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
                                                                                     d_brpt, d_bcol, d_bval,
                                                                                     d_crpt, d_ccol, d_cval,
                                                                                     bin->d_row_perm, bin->d_row_nz,
                                                                                     bin->bin_offset[i], bin->bin_size[i]);
            ...
            case 6:
            {
                ...
                GS = bin->bin_size[i];
                calculate_value_col_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>(d_arpt, d_acol, d_aval,
                                                                               d_brpt, d_bcol, d_bval,
                                                                               d_crpt, d_ccol, d_cval,
                                                                               bin->d_row_perm, bin->d_row_nz,
