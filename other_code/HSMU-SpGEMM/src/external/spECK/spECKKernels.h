...
{
public:
	spECKKernels(uint32_t blockDim = 128) : blockDim{blockDim}
	{
	}
	...
	// #####################################################################
	// Numeric Hash SpGEMM
	template <typename INDEX_TYPE, class GlobalMap, uint32_t SHARED_HASH_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
	void h_HashSpGEMMNumeric(dCSRNoDealloc<double> matA, dCSRNoDealloc<double> matB, dCSRNoDealloc<double> matC, GlobalMap *maps, INDEX_TYPE mapCount,
	...
	template <typename INDEX_TYPE, class GlobalHashMap, class GlobalRowOffsetMap, uint32_t SHARED_HASH_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
	void h_SpGEMMNumericLauncher(dCSRNoDealloc<double> matA, dCSRNoDealloc<double> matB, dCSRNoDealloc<double> matC,
	...
	template <typename INDEX_TYPE, class GlobalMap, uint32_t SHARED_HASH_SIZE, bool SUPPORT_GLOBAL, uint32_t THREADS>
	void h_DenseSpGEMMNumeric(dCSRNoDealloc<double> matA, dCSRNoDealloc<double> matB, dCSRNoDealloc<double> matC, GlobalMap *maps, INDEX_TYPE mapCount,
	...
	template <typename INDEX_TYPE, uint32_t THREADS, uint32_t entriesPerBlock>
	void h_HashSpGEMMSorting(dCSRNoDealloc<double> matC, INDEX_TYPE *blockStartRow, uint32_t numberBlocks, bool bitShiftNumRows);
